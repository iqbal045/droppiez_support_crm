...
<Head title="Login" />
        <div class="p-6 min-h-screen flex justify-center items-center bg-[#f5f5f5]">
            ...
            <div class="w-full max-w-lg">
                <form
                    class="auth mt-8 bg-white dark:bg-slate-900 border border-gray-100 shadow-xl rounded-lg overflow-hidden"
                    @submit.prevent="login">
                    <div class="px-8 py-5">
                        <Link :href="route('home')">
                        <logo class="block w-48 mx-auto fill-white mt-10" />
                        </Link>

                        <!-- <div class="mx-auto mt-2 w-24 border-b" /> -->

                        <h2 class="font-black text-[24px] mt-10">{{ __('Login') }}</h2>
                        <text-input v-model="form.email" :error="form.errors.email" class="mt-5" label="Email"
                        ...
                        <loading-button :disabled="disable_login_button && site_key" :loading="form.processing"
                            class="ml-auto btn-indigo bg-custom-dark hover:bg-custom-dark w-full items-center justify-center mt-4" type="submit"
                        ...
                        <div class="mt-5 flex justify-center">
                            <Link class="ml-2 text-[14px]" :href="route('password.reset')">{{ __('Forgot your password?') }}</Link>
                        </div>
                        <div class="mt-5 text-[14px] flex justify-center">
                            Don’t have account?
                            <Link class="ml-2 text-[14px]" :href="route('register')">{{ __('Register') }}</Link>
                        </div>
                    </div>
                    <div v-if="is_demo"
                        class="pb-2 bg-white dark:bg-slate-900 border-t border-gray-100 dark:border-gray-700 flex flex-col gap-1 login-as items-center">
                        ...
                        <div class="action flex flex-col sm:flex-row gap-3">
                            ...
                            <button class=" btn-indigo" @click="autofillLogin($event, 'customer', true)">
                                Customer
                            </button>
                        </div>
                    </div>
